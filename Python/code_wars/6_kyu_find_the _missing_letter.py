# Find the missing letter
#
# Write a method that takes an array of consecutive (increasing) letters as input and that returns the missing
#   letter in the array.
#
# You will always get an valid array. And it will be always exactly one letter be missing. The length of the
#   array will always be at least 2.
# The array will always contain letters in only one case.
#
# Example:
# ['a','b','c','d','f'] -> 'e' ['O','Q','R','S'] -> 'P'
# (Use the English alphabet with 26 letters!)


def find_missing_letter(chars):
    keys = [x.upper() if chars[0].isupper() else x.lower() for x in "abcdefghijklmnopqrstuvwxyz"]
    val = range(1, 27)
    my_dict = dict(zip(keys, val))

    values = [my_dict.get(x) for x in chars]
    new_dict = dict(zip(val, keys))

    for x in values[:-1:]:
        if values[values.index(x) + 1] - values[values.index(x)] == 2:
            return new_dict.get(x + 1)
