...
NOT_VALID_CHARACTERS = " /,.'_?;:*!-#\\"


def top_3_words(text):
    # get rid of \' escape characters, by replacing ' with "
    text = text.replace("'", '"')

    # append to the text_filtered a character if it meets conditions
    text_filtered = ""
    char: str
    for i, char in enumerate(text):
        if char in NOT_VALID_CHARACTERS:
            text_filtered += " "
        elif char == '"':
    ...
    # make apostrophes back ' from " and make all characters lower
    text_filtered = text_filtered.replace('"', "'").lower()

    # make a list
    text_split = text_filtered.split()

    # find 3 most common elements (or less)
    most_common = Counter(text_split).most_common(3)
